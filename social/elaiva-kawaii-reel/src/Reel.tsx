...
function Kawaii({x,y,scale=1,rotation=0,face='happy',sweat=false,coffee=false,sandwich=false,walkPhase=0,mouthOpen=false,armsAround=false}:KawaiiProps){
  ...
  return <div style={{position:'absolute',left:x,top:y,transform:`translate(-50%,-50%) scale(${scale}) rotate(${rotation}deg)`,width:250,height:310,zIndex:2,perspective:600}}>
    ...
    <div style={{position:'absolute',left:20,top:38,width:210,height:205,
      ...
      borderRadius:'46% 46% 42% 42%',border:'5px solid #eadfd6',
      boxShadow:'inset -14px -18px 0 rgba(190,165,150,.20), 0 18px 35px #0002',zIndex:1,
    ...
    {/* arms */}
    <div style={{position:'absolute',left:-12,top:166,width:86,height:25,background:'linear-gradient(#fff,#eee4dc)',border:'4px solid #eadfd6',borderRadius:30,transform:`rotate(${armsAround?-8:-12}deg)`,zIndex:4}}/>
    <div style={{position:'absolute',right:-12,top:166,width:86,height:25,background:'linear-gradient(#fff,#eee4dc)',border:'4px solid #eadfd6',borderRadius:30,transform:`rotate(${armsAround?8:12}deg)`,zIndex:4}}/>
    <div style={{position:'absolute',left:-5,top:166,width:25,height:25,background:CREAM,border:'4px solid #eadfd6',borderRadius:'50%',zIndex:5}}/>
    <div style={{position:'absolute',right:-5,top:166,width:25,height:25,background:CREAM,border:'4px solid #eadfd6',borderRadius:'50%',zIndex:5}}/>

    {/* legs */}
    <div style={{position:'absolute',left:73,top:229,width:30,height:62,background:'linear-gradient(90deg,#fff,#eee5dd)',border:'4px solid #eadfd6',borderRadius:20,transform:`rotate(${4+legA}deg)`,transformOrigin:'top center',zIndex:0}}/>
    <div style={{position:'absolute',right:73,top:229,width:30,height:62,background:'linear-gradient(90deg,#fff,#eee5dd)',border:'4px solid #eadfd6',borderRadius:20,transform:`rotate(${-4+legB}deg)`,transformOrigin:'top center',zIndex:0}}/>
...
export default function Reel(){
 ...
 // 8 seconds of tsamikos-style circle dancing, with linked arms.
 const danceProgress=Math.max(0,Math.min(1,(f-250)/240));
 const circleAngle=danceProgress*Math.PI*4;
 ...
 return <Scene><AbsoluteFill style={{background:bg}}>
  ...
  {sec>=8.33&&sec<16.33&&<>
    ...
    {dancers.map((d,i)=>{const next=dancers[(i+1)%4];return <React.Fragment key={i}><Kawaii x={d.x} y={d.y+55*Math.sin(f/4+i)} scale={i%2?.64:.70} rotation={d.r+10*Math.sin(f/5+i)} armsAround walkPhase={f/4+i}/><LinkArms x1={d.x+(next.x-d.x)*.12} y1={d.y+(next.y-d.y)*.12-15} x2={next.x-(next.x-d.x)*.12} y2={next.y-(next.y-d.y)*.12-15}/></React.Fragment>})}
  ...
  {sec>=20&&<div style={{position:'absolute',inset:0,background:'#fbf4ed'}}>
    {/* The original white character comes from deep in the background, walking toward camera. */}
